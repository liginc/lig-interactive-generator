...
inquirer
  ...
  .then((answer) => {
    ...
    thePromise
      ...
      .then(() => {
        download(
          "https://github.com/liginc/laravel-mix-boilerplate-wordpress.git",
          { mergeReadme: false }
        );
      })
      .then(() => {
        if (answer.wordpress_type == "with theme") {
          ...
          fs.removeSync(
            path.join(rootDir, "wp/wp-content/themes/input-theme-name")
          );
          download("https://github.com/liginc/lig-wordpress-template.git")
            .then(() => {
              fs.renameSync(
                path.join(
                  rootDir,
                  "wp/wp-content/themes/lig-wordpress-template"
                ),
                path.join(rootDir, "wp/wp-content/themes", projectName)
              );
              fs.renameSync(
                path.join(rootDir, "resources/themes/lig-wordpress-template"),
                path.join(rootDir, "resources/themes", projectName)
              );
            })
            .then(() => {
              download(
                "https://github.com/liginc/lig-wordpress-functions.git",
                {
          ...
          fs.renameSync(
            path.join(rootDir, "resources/themes/input-theme-name"),
            path.join(rootDir, "resources/themes", projectName)
          );
      ...
      .then(() => {
        ...
        childProcess.spawnSync("sed", [
          ...
          "",
          "-e",
          "s|PHP_VER=.*$|PHP_VER=" + answer.php_ver + "|g",
          env,
        ]);
        childProcess.spawnSync("sed", [
          "-i",
          "",
          "-e",
          "s|MYSQL_VER=.*$|MYSQL_VER=" + answer.mysql_ver + "|g",
          env,
        ]);
        if (answer.wordpress_ver !== "") {
          childProcess.spawnSync("sed", [
            ...
            "",
            "-e",
            "s|WP_VERSION=.*$|WP_VERSION=" + answer.wordpress_ver + "|g",
            env,
          ]);
        } else {
          const wpLatestVersion = childProcess.spawnSync(
            "git",
            [
              "ls-remote",
              "--tags",
          ...
          childProcess
            .spawnSync("sed", [
              ...
              "",
              "-e",
              "s|WP_VERSION=.*$|WP_VERSION=" +
                wpLatestVersion.stdout.toString().replace(/\r?\n/g, "") +
                "|g",
              env,
            ])
            .stderr.toString();
          process.stdout.write(
            "WP version will set to " + wpLatestVersion.stdout.toString() + "\n"
          );
        }
        ...
        childProcess.spawnSync("sed", [
          ...
          "",
          "-e",
          "s|WP_THEME_NAME=.*$|WP_THEME_NAME=" + projectName + "|g",
          env,
        ]);
        childProcess.spawnSync("sed", [
          "-i",
        ...
        fs.readFile(webpackMixJs, "utf8", function (err, data) {
          ...
          data = data.replace(
            /(const srcRelativePath =).+?\((.+?)\s.+?.replace\(.+?\)/s,
            "$1 $2;"
          );
          data = data.replace(
            /(const distRelativePath =).+?\((.+?)\s.+?.replace\(.+?\)/s,
            "$1 $2;"
          );
          data = data.replace(
            /(^\s+?\.sass\($)/m,
