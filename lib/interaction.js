...
const currentNodeVer = process.env.npm_config_node_version;
...
module.exports = function (argv) {
    inquirer.prompt([questionType, questionName, questionNodeVersion]).then((answers) => {
        ...
        fs.renameSync(path.join(__dirname, '../default-assets', answers.type, '_gitignore'), path.join(projectDir, '.gitignore'));

        const scriptPath = path.join(__dirname, '../script', answers.type + '.js');

        if (projectNode != "stable") {
            CreateEnvironment(scriptPath, projectName, projectNode)
        } else {
            stable.then((version) => {
